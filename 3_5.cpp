...
void *threadFunc(void *param)
{
        ...
        int t_id=p->t_id;
        for(int i=t_id;i<bin_a_v.size();i+=worker_count)
        {
            //a_first1.insert(a_first1.begin()+i,find_first(bin_a_v[i]));
            a_first1[i]=find_first(bin_a_v[i]);
        }
        pthread_exit(NULL);
}
...
int main()
{
    ...
    for(int i=0;i<bin_a_v.size();i++)
        a_first1.insert(a_first1.begin(),-1);
    //由于每个消元子在进行消元的过程中，首个非零元素的位置不会改变，可以先将其求出来
    //对其进行并行化操作
    // for(int i=0;i<bin_a_v.size();i++)
    // {
    //     a_first1.insert(a_first1.end(),find_first(bin_a_v[i]));
    // }
