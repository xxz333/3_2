...
//矩阵行数
const int row_num_1 = 170;
const int row_num_2 = 53;
int row_n_1 = row_num_1;//消元子
int row_n_2 = row_num_2;//被消元行
int col_num = 562;//矩阵列数
//用于存放读入的十进制的消元子和被消元行
int** a, ** b;
void malloc_ab();
//将数据读入内存
...
void *threadFunc(void* param)
{
    ...
    int t_id=p->t_id;
    int i;
    for(i=t_id;i < bin_b_v.size();i+=worker_count)
    {
...
void handle_one(unsigned int* k1, unsigned int* k2, int n_bei, bool& judge1, bool& judge2)//k1->消元子,k2->被消元行
{
    ...
    unsigned int* tem = new unsigned int[bin_col_num];
    for (int i = 0; i < bin_col_num; i++)
    {
        temp_k1[i] = k1[i];
        temp_k2[i] = k2[i];
    ...
    /*
    int i;
    for(i=0;i<bin_col_num;i+=4)
    {
        //将四个unsigned int类型的数从内存加载到向量寄存器中
        int32x4_t vtem1=vldlq_s32(&temp_k1[i]);
        int32x4_t vtem2=vldlq_s32(&temp_k2[i]);
        //定义一个向量寄存器，将其初始化为1
        unsigned int temm=1;
        int32x4_t tem1=vldlq_s32(&temm);
        //vtem<-vtem&1
        vtem1=Int32x4.and(vtem1,tem1);
        vtem2=Int32x4.and(vtem2,tem1);
        //tem_v <- vtem1^vtem2
        int32x4_t tem_v=vldlq_s32(&temm);
        tem_v=Int32x4.xor(vtem1,vtem2);
        //将四个数从向量寄存器存储到内存
        vst1q_s32(&tem[i],tem_v);
    }
    //行尾还有几个元素没计算
    for(;i<bin_col_num;i++)
        tem[i] = (temp_k1[i] & 1) ^ (temp_k2[i] & 1);
